...
public class BoogleMapsApplication {
    ...
    /**
     * Save the ADDRESSES details to H2 DB
     * @param repository
     * @return
     */
    @Bean
    CommandLineRunner initDatabase(MockAddressRepository repository) {
    ...
    return args ->{
    	 Arrays.asList(ADDRESSES).forEach(add->{
             ...
             Address addObj= new Address(streetAndNumber, city, state, zip);
             addObj.setLat(counter.getAndDecrement()+0d);
             addObj.setLon(counter.getAndDecrement()+0d);
